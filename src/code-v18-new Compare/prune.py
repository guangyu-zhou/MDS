...
def enumPseudoClique(K, V_remain, E, V, AdjList, theta, max_dense, dense, Ancestors, CoveredV):
	...
	for i in range(len(V_remain_in_seq)):
		...
		K_new.append(v)
 		...
 		# print "K_new",K_new
 		start_time = time.time()
		new_dense = util.computeDensity(set(K_new), E, isWeight = False)
...
def main():
	...
	print "========================="
	print "top-k",k,"threshold", theta, "With Prune"
	print "========================="
	coverage_set = set()
	max_dense =  S[maxV]
...
if(len(sys.argv) != 5):
	...
	theta = 0.5
	min_elem_output = 1
	print "Running default... k=",k, "theta=",theta, "min_elem_output=",min_elem_output
