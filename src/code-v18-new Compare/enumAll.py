...
# 	# print "Result: v,len(S[v]),len(covered_v),thre"
# 	for v in O:
# 		# print "====v====", v
...
# 			num_no_full +=1
# 			all_full = False
# 			# print "v", v, "threshold",(v/2.0 - len(covered_v))/(k - len(S[v]))
# 			# print v,len(S[v]),len(covered_v),(v/2.0 - len(covered_v))/(k - len(S[v]))
...
# 		# else:
# 			# print "v", v, "full"# threshold",(v/2.0 - len(covered_v))/(k - len(S[v]))
...
# 	if len(S[v]) < k:
# 		# thre = (v/2.0 - len(covered_v))/((k - len(S[v]))*4)
...
def comp_Potential(K, V_remain, E, theta, x, N_r, Union, AdjList):
	...
	# print "N_r_S",N_r, "N_x",N_x, "N_r_x", N_r_x
...
def selectTopK(max_dense, k):
	print "Top",k,"mds"
	mdsList = []
	for mds in max_dense:
		mdsList.append(mds[0])
	# print mdsList
	mdsList.sort(key = len, reverse = True)
	for i in range(k):
		# print mdsList[i] #, len(mdsList[i])
		for elem in mdsList[i]:
			print elem,
		print
...
def main():
	...
	print "========================="
	print "max_dense size,", len(max_dense)

	# selectTopK(max_dense, k = 105)
